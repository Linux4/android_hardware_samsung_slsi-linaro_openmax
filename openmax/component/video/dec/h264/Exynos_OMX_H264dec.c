    ...
    EXYNOS_OMX_BASECOMPONENT *pExynosComponent)
{
    ...
    case MFC_1501:
    case MFC_150:
        pH264Dec->hMFCH264Handle.maxLevel = OMX_VIDEO_AVCLevel6;
        break;
    case MFC_1400:
    case MFC_1410:
    case MFC_140:
...
void H264CodecUpdateHdrInfo(OMX_COMPONENTTYPE *pOMXComponent)
{
    ...
    ExynosVideoDecOps  *pDecOps     = pH264Dec->hMFCH264Handle.pDecOps;
    ExynosVideoHdrInfo *pHdrInfo    = &(pH264Dec->hMFCH264Handle.sHdrInfo);
    ...
    ExynosVideoMeta     *pMeta)
{
    ...
    /* interlace */
    {
        if (pH264Dec->hMFCH264Handle.codecOutbufConf.bInterlaced == VIDEO_TRUE) {
            Exynos_OSAL_Log(EXYNOS_LOG_TRACE, "[%p][%s] interlace type = %x",
                                pExynosComponent, __FUNCTION__, pH264Dec->hMFCH264Handle.interlacedType);
            pMeta->eType |= VIDEO_INFO_TYPE_INTERLACED;
            pMeta->data.dec.nInterlacedType = pH264Dec->hMFCH264Handle.interlacedType;
        }
    }
    ...
    /* HDR */
    {
        ...
        if (pH264Dec->hMFCH264Handle.sHdrInfo.eValidType & (HDR_INFO_COLOR_ASPECTS | HDR_INFO_RANGE)) {
            ExynosVideoColorAspects *pColorAspects = &(pH264Dec->hMFCH264Handle.sHdrInfo.sColorAspects);
        ...
        if (pH264Dec->hMFCH264Handle.sHdrInfo.eValidType & (HDR_INFO_LIGHT | HDR_INFO_LUMINANCE)) {
            ExynosVideoHdrStatic *pHDRStaticInfo = &(pH264Dec->hMFCH264Handle.sHdrInfo.sHdrStatic);
    ...
    {
        if (pH264Dec->hMFCH264Handle.nActualFormat != 0) {
            pMeta->nPixelFormat  = pH264Dec->hMFCH264Handle.nActualFormat;
            pMeta->eType        |= VIDEO_INFO_TYPE_CHECK_PIXEL_FORMAT;

            pH264Dec->hMFCH264Handle.nActualFormat = 0;
        } else {
...
OMX_ERRORTYPE H264CodecUpdateResolution(OMX_COMPONENTTYPE *pOMXComponent)
{
    ...
    if (pH264Dec->hMFCH264Handle.MFCOutputColorType != pH264Dec->hMFCH264Handle.codecOutbufConf.eColorFormat) {
        OMX_COLOR_FORMATTYPE eOutputFormat = Exynos_OSAL_Video2OMXFormat(pH264Dec->hMFCH264Handle.codecOutbufConf.eColorFormat);

        Exynos_OSAL_Log(EXYNOS_LOG_INFO, "[%p][%s] The format(%x) is changed to %x by H/W Codec",
                                    pExynosComponent, __FUNCTION__,
                                    pH264Dec->hMFCH264Handle.MFCOutputColorType,
                                    pH264Dec->hMFCH264Handle.codecOutbufConf.eColorFormat);

        pH264Dec->hMFCH264Handle.MFCOutputColorType = pH264Dec->hMFCH264Handle.codecOutbufConf.eColorFormat;
        Exynos_SetPlaneToPort(pOutputPort, Exynos_OSAL_GetPlaneCount(eOutputFormat, pOutputPort->ePlaneType));

        bFormatChanged = OMX_TRUE;

        if (!(pH264Dec->hMFCH264Handle.codecOutbufConf.eFilledDataType & DATA_TYPE_8BIT_SBWC)) {
    ...
    switch (pH264Dec->hMFCH264Handle.codecOutbufConf.eFilledDataType) {
    case DATA_8BIT_SBWC:
    ...
    if (pOutputPort->bufferProcessType & (BUFFER_COPY | BUFFER_COPY_FORCE)) {
        ...
        if ((pVideoDec->bReconfigDPB) ||
            ...
            ((OMX_S32)pOutputPortDefinition->nBufferCountMin < pH264Dec->hMFCH264Handle.maxDPBNum)) {
            pOutputPort->exceptionFlag = NEED_PORT_DISABLE;
...
OMX_ERRORTYPE H264CodecHeaderDecoding(OMX_COMPONENTTYPE *pOMXComponent, EXYNOS_OMX_DATA *pSrcInputData)
{
    OMX_ERRORTYPE                  ret               = OMX_ErrorNone;
    EXYNOS_OMX_BASECOMPONENT      *pExynosComponent  = (EXYNOS_OMX_BASECOMPONENT *)pOMXComponent->pComponentPrivate;
    EXYNOS_OMX_VIDEODEC_COMPONENT *pVideoDec         = (EXYNOS_OMX_VIDEODEC_COMPONENT *)pExynosComponent->hComponentHandle;
...
OMX_ERRORTYPE H264CodecSrcSetup(OMX_COMPONENTTYPE *pOMXComponent, EXYNOS_OMX_DATA *pSrcInputData)
{
    ...
    if (pH264Dec->hMFCH264Handle.videoInstInfo.supportInfo.dec.bPrioritySupport == VIDEO_TRUE)
    ...
    ret = H264CodecHeaderDecoding(pOMXComponent, pSrcInputData);
    if (ret != OMX_ErrorNone) {
        pInbufOps->Cleanup_Buffer(hMFCHandle);
        goto EXIT;
    }
...
OMX_ERRORTYPE H264CodecDstSetup(OMX_COMPONENTTYPE *pOMXComponent)
{
    ...
    if (pH264Dec->hMFCH264Handle.videoInstInfo.supportInfo.dec.bDrvDPBManageSupport != VIDEO_TRUE) {
...
OMX_ERRORTYPE Exynos_H264Dec_SrcIn(OMX_COMPONENTTYPE *pOMXComponent, EXYNOS_OMX_DATA *pSrcInputData)
{
    ...
    if (((pExynosComponent->codecType == HW_VIDEO_DEC_SECURE_CODEC) ||
        ...
        if (pVideoDec->bQosChanged == OMX_TRUE) {
            if (pH264Dec->hMFCH264Handle.videoInstInfo.supportInfo.dec.bOperatingRateSupport == VIDEO_TRUE) {
            ...
            if (nVideoFormat != VIDEO_COLORFORMAT_UNKNOWN) {
                ...
                if (nOMXFormat != OMX_COLOR_FormatUnused) {
                    ...
                    if (nPixelFormat != 0) {
                        ...
                        pH264Dec->hMFCH264Handle.nActualFormat = nPixelFormat;
                    }
                }
            }
        }
...
OSCL_EXPORT_REF OMX_ERRORTYPE Exynos_OMX_ComponentInit(OMX_HANDLETYPE hComponent, OMX_STRING componentName)
{
    ...
    Exynos_OSAL_Log(EXYNOS_LOG_ESSENTIAL, "[%p][%s] GetInstInfo for dec skype(%d), DrvDPBManaging(%d)", pExynosComponent, __FUNCTION__,
...
#ifdef USE_ANDROID
#ifdef USE_SKYPE_HD
    Exynos_OSAL_AddVendorExt(hComponent, "rtc-ext-dec-caps-vt-driver-version", (OMX_INDEXTYPE)OMX_IndexSkypeParamDriverVersion);
