...
void Mpeg2CodecUpdateHdrInfo(OMX_COMPONENTTYPE *pOMXComponent)
{
    EXYNOS_OMX_BASECOMPONENT      *pExynosComponent  = (EXYNOS_OMX_BASECOMPONENT *)pOMXComponent->pComponentPrivate;
    EXYNOS_OMX_VIDEODEC_COMPONENT *pVideoDec         = (EXYNOS_OMX_VIDEODEC_COMPONENT *)pExynosComponent->hComponentHandle;
    EXYNOS_MPEG2DEC_HANDLE        *pMpeg2Dec         = (EXYNOS_MPEG2DEC_HANDLE *)pVideoDec->hCodecHandle;
    ...
    ExynosVideoHdrInfo *pHdrInfo    = &(pMpeg2Dec->hMFCMpeg2Handle.sHdrInfo);
...
OMX_ERRORTYPE Mpeg2CodecUpdateResolution(OMX_COMPONENTTYPE *pOMXComponent)
{
    ...
    if (pMpeg2Dec->hMFCMpeg2Handle.MFCOutputColorType != pMpeg2Dec->hMFCMpeg2Handle.codecOutbufConf.eColorFormat) {
        ...
        if (!(pMpeg2Dec->hMFCMpeg2Handle.codecOutbufConf.eFilledDataType & DATA_TYPE_8BIT_SBWC)) {
...
OMX_ERRORTYPE Mpeg2CodecHeaderDecoding(OMX_COMPONENTTYPE *pOMXComponent, EXYNOS_OMX_DATA *pSrcInputData)
{
    OMX_ERRORTYPE                  ret               = OMX_ErrorNone;
    EXYNOS_OMX_BASECOMPONENT      *pExynosComponent  = (EXYNOS_OMX_BASECOMPONENT *)pOMXComponent->pComponentPrivate;
    EXYNOS_OMX_VIDEODEC_COMPONENT *pVideoDec         = (EXYNOS_OMX_VIDEODEC_COMPONENT *)pExynosComponent->hComponentHandle;
...
OMX_ERRORTYPE Mpeg2CodecSrcSetup(OMX_COMPONENTTYPE *pOMXComponent, EXYNOS_OMX_DATA *pSrcInputData)
{
    ...
    if (pMpeg2Dec->hMFCMpeg2Handle.videoInstInfo.supportInfo.dec.bPrioritySupport == VIDEO_TRUE)
    ...
    ret = Mpeg2CodecHeaderDecoding(pOMXComponent, pSrcInputData);
    if (ret != OMX_ErrorNone) {
        pInbufOps->Cleanup_Buffer(hMFCHandle);
        goto EXIT;
    }
...
OMX_ERRORTYPE Exynos_Mpeg2Dec_SrcIn(OMX_COMPONENTTYPE *pOMXComponent, EXYNOS_OMX_DATA *pSrcInputData)
{
    ...
    if (((bInStartCode = Check_Mpeg2_StartCode(pSrcInputData->buffer.addr[0], oneFrameSize)) == OMX_TRUE) ||
        ...
        if (pVideoDec->bQosChanged == OMX_TRUE) {
            if (pMpeg2Dec->hMFCMpeg2Handle.videoInstInfo.supportInfo.dec.bOperatingRateSupport == VIDEO_TRUE) {
...
OMX_ERRORTYPE Exynos_Mpeg2Dec_SrcOut(OMX_COMPONENTTYPE *pOMXComponent, EXYNOS_OMX_DATA *pSrcOutputData)
{
    ...
    FunctionIn();
    if (pMpeg2Dec->hMFCMpeg2Handle.bConfiguredMFCSrc == OMX_TRUE) {
   ...
   if ((pVideoDec->bThumbnailMode == OMX_FALSE) &&
    ...
    /* update extra info */
    {
        ...
        if (pVideoBuffer->frameType & VIDEO_FRAME_WITH_HDR_INFO)
            Mpeg2CodecUpdateHdrInfo(pOMXComponent);
