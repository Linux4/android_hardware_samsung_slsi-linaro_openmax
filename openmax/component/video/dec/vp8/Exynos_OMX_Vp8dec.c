...
OMX_ERRORTYPE VP8CodecUpdateExtraInfo(
    OMX_COMPONENTTYPE   *pOMXComponent,
    ExynosVideoMeta     *pMeta)
{
    ...
    {
        if (pVp8Dec->hMFCVp8Handle.nActualFormat != 0) {
            pMeta->nPixelFormat  = pVp8Dec->hMFCVp8Handle.nActualFormat;
            pMeta->eType        |= VIDEO_INFO_TYPE_CHECK_PIXEL_FORMAT;

            pVp8Dec->hMFCVp8Handle.nActualFormat = 0;
        } else {
...
OMX_ERRORTYPE Vp8CodecUpdateResolution(OMX_COMPONENTTYPE *pOMXComponent)
{
    ...
    if (pVp8Dec->hMFCVp8Handle.MFCOutputColorType != pVp8Dec->hMFCVp8Handle.codecOutbufConf.eColorFormat) {
        OMX_COLOR_FORMATTYPE eOutputFormat = Exynos_OSAL_Video2OMXFormat(pVp8Dec->hMFCVp8Handle.codecOutbufConf.eColorFormat);

        Exynos_OSAL_Log(EXYNOS_LOG_INFO, "[%p][%s] The format(%x) is changed to %x by H/W Codec",
                                    pExynosComponent, __FUNCTION__,
                                    pVp8Dec->hMFCVp8Handle.MFCOutputColorType,
                                    pVp8Dec->hMFCVp8Handle.codecOutbufConf.eColorFormat);

        pVp8Dec->hMFCVp8Handle.MFCOutputColorType = pVp8Dec->hMFCVp8Handle.codecOutbufConf.eColorFormat;
        Exynos_SetPlaneToPort(pOutputPort, Exynos_OSAL_GetPlaneCount(eOutputFormat, pOutputPort->ePlaneType));

        bFormatChanged = OMX_TRUE;

        if (!(pVp8Dec->hMFCVp8Handle.codecOutbufConf.eFilledDataType & DATA_TYPE_8BIT_SBWC)) {
    ...
    switch (pVp8Dec->hMFCVp8Handle.codecOutbufConf.eFilledDataType) {
    case DATA_8BIT_SBWC:
    ...
    if (pOutputPort->bufferProcessType & (BUFFER_COPY | BUFFER_COPY_FORCE)) {
        ...
        if ((pVideoDec->bReconfigDPB) ||
            ...
            ((OMX_S32)pOutputPortDefinition->nBufferCountMin < pVp8Dec->hMFCVp8Handle.maxDPBNum)) {
            pOutputPort->exceptionFlag = NEED_PORT_DISABLE;
...
OMX_ERRORTYPE VP8CodecHeaderDecoding(OMX_COMPONENTTYPE *pOMXComponent, EXYNOS_OMX_DATA *pSrcInputData)
{
    OMX_ERRORTYPE                  ret               = OMX_ErrorNone;
    EXYNOS_OMX_BASECOMPONENT      *pExynosComponent  = (EXYNOS_OMX_BASECOMPONENT *)pOMXComponent->pComponentPrivate;
    EXYNOS_OMX_VIDEODEC_COMPONENT *pVideoDec         = (EXYNOS_OMX_VIDEODEC_COMPONENT *)pExynosComponent->hComponentHandle;
...
OMX_ERRORTYPE VP8CodecSrcSetup(OMX_COMPONENTTYPE *pOMXComponent, EXYNOS_OMX_DATA *pSrcInputData)
{
    ...
    if (pVp8Dec->hMFCVp8Handle.videoInstInfo.supportInfo.dec.bPrioritySupport == VIDEO_TRUE)
    ...
    ret = VP8CodecHeaderDecoding(pOMXComponent, pSrcInputData);
    if (ret != OMX_ErrorNone) {
        pInbufOps->Cleanup_Buffer(hMFCHandle);
        goto EXIT;
    }
...
OMX_ERRORTYPE VP8CodecDstSetup(OMX_COMPONENTTYPE *pOMXComponent)
{
    ...
    if (pVp8Dec->hMFCVp8Handle.videoInstInfo.supportInfo.dec.bDrvDPBManageSupport != VIDEO_TRUE) {
...
OMX_ERRORTYPE Exynos_VP8Dec_SrcIn(OMX_COMPONENTTYPE *pOMXComponent, EXYNOS_OMX_DATA *pSrcInputData)
{
    ...
    if (((bInStartCode = Check_VP8_StartCode(pSrcInputData->buffer.addr[0], oneFrameSize)) == OMX_TRUE) ||
        ...
        if (pVideoDec->bQosChanged == OMX_TRUE) {
            if (pVp8Dec->hMFCVp8Handle.videoInstInfo.supportInfo.dec.bOperatingRateSupport == VIDEO_TRUE) {
...
OMX_ERRORTYPE Exynos_VP8Dec_SrcOut(OMX_COMPONENTTYPE *pOMXComponent, EXYNOS_OMX_DATA *pSrcOutputData)
{
    ...
    if (pVp8Dec->hMFCVp8Handle.bConfiguredMFCSrc == OMX_TRUE) {
...
OMX_ERRORTYPE Exynos_VP8Dec_DstOut(OMX_COMPONENTTYPE *pOMXComponent, EXYNOS_OMX_DATA *pDstOutputData)
{
   ...
   if ((pVideoDec->bThumbnailMode == OMX_FALSE) &&
            ...
            if (nVideoFormat != VIDEO_COLORFORMAT_UNKNOWN) {
                ...
                if (nOMXFormat != OMX_COLOR_FormatUnused) {
                    ...
                    if (nPixelFormat != 0) {
                        ...
                        pVp8Dec->hMFCVp8Handle.nActualFormat = nPixelFormat;
                    }
                }
            }
        }
    ...
    if ((pOutputPort->bufferProcessType == BUFFER_SHARE) &&
        ...
        VP8CodecUpdateExtraInfo(pOMXComponent, pVideoBuffer->planes[2].addr);
    }
...
OMX_ERRORTYPE Exynos_VP8Dec_srcOutputBufferProcess(OMX_COMPONENTTYPE *pOMXComponent, EXYNOS_OMX_DATA *pSrcOutputData)
{
    ...
    if (pExynosInputPort->bufferProcessType & (BUFFER_COPY | BUFFER_COPY_FORCE)) {
